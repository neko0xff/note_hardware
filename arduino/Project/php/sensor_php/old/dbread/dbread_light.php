...
if(!empty($_POST['default_light'])){
        ...
        /*把資料上傳到資料庫的表格上*/
	    $post_sql = "INSERT INTO sensor_light (default_light,date, time) VALUES ('".$default_light."','".$d."','".$t."')"; 
...
/*把資料庫的表格叫出來*/
$sql = "SELECT id, light, date, time FROM sensor_light";  // Update your tablename here
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        echo "<strong> Id:</strong> " . $row["id"]. " &nbsp <strong>light:</strong> " . $row["light"]. " &nbsp   <strong>Date:</strong> " . $row["date"]." &nbsp <strong>Time:</strong>" .$row["time"]. "<p>";
    }
}else{
    echo "0 results";
}
