...
if(!empty($_POST['default_soil'])){
        ...
        /*把資料上傳到資料庫的表格上*/
	    $post_sql = "INSERT INTO sensor_soil (default_soil,date, time) VALUES ('".$default_soil."','".$d."','".$t."')"; 
...
/*把資料庫的表格叫出來*/
$sql = "SELECT id, soil, date, time FROM sensor_soil";  // Update your tablename here
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        echo "<strong> Id:</strong> " . $row["id"]. " &nbsp <strong>soil:</strong> " . $row["soil"]. " &nbsp   <strong>Date:</strong> " . $row["date"]." &nbsp <strong>Time:</strong>" .$row["time"]. "<p>";
    }	
}else{
    echo "0 results";
}
